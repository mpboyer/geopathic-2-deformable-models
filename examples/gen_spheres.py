...
def build_sphere(level: int) -> tuple[np.ndarray, list, int]:
    ...
    np_idx = int(np.argmin(np.linalg.norm(verts - [0.0, 0.0, 1.0], axis=1)))
    ...
    np_idx = int(np.argmin(np.linalg.norm(verts - [0.0, 0.0, 1.0], axis=1)))
    assert (
        np.linalg.norm(verts[np_idx] - [0.0, 0.0, 1.0]) < 1e-8
...
def write_obj(
    path: Path, verts: np.ndarray, faces: list, source_idx: int, h: float
) -> None:
    with open(path, "w") as f:
        ...
        f.write(
            "# ground-truth distance: arccos(v_z + 1)  "
...
def main():
    ...
    for level in sorted(args.levels):
        ...
        assert (
            np.linalg.norm(verts[src_idx]) < 1e-10
        ...
        print(
            f"level {level:2d} | {n_v:6d} vertices | {n_f:6d} faces "
