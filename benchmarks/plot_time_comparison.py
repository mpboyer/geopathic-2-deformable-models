...
agg_v = data.groupby(["variant", "vertices"])["time_s"].median().reset_index()
...
plt.close()
print(
    f"Saved: benchmarks/pdf/plot3a_timing_loglog.pdf  (slope={slope:.3f}, R²={r_value**2:.4f})"
)

agg_m = data.groupby(["model", "variant"])["time_s"].median().reset_index()
...
fig_b, ax_b = plt.subplots(
    figsize=(
        max(8, pivot_norm.shape[1] * 0.35 + 2),
...
agg_src = data.groupby(["model", "source", "variant"])["time_s"].median().reset_index()
